...
namespace TutorialBoss
{
    ...
    public class TutorialGroggyState : IBossState
    {
        ...
        {
            // ── Phase 1: 코어 낙하 (높은 위치 → 바닥 위치) ──────────────
            Debug.Log($"[TutorialBoss Groggy] 코어 낙하: {tb.CoreNormalY} → {tb.CoreGroggyY}");
            yield return tb.StartCoroutine(tb.MoveCoreToY(tb.CoreGroggyY, tb.CoreMoveDuration));
            ...
            // ── Phase 3: 코어 복귀 (바닥 위치 → 높은 위치) ──────────────
            Debug.Log($"[TutorialBoss Groggy] 코어 복귀: {tb.CoreGroggyY} → {tb.CoreNormalY}");
            yield return tb.StartCoroutine(tb.MoveCoreToY(tb.CoreNormalY, tb.CoreMoveDuration));
