...
## 해결 기록
- 해결일: 2026-09-14
- 수정 커밋: 37b24b5 (검증 스크립트와 버그 기록), bab5f21 (Check.md 원문/대체 판정 기록)
- 구현: verify-static.cjs로 원문 행 수와 대체 판정을 분리. Check.md에 원문 18/22 PASS, 4 FAIL 및 대체 22/22 PASS 기록.
- 커밋 후 재검증: verify-static.cjs 실행 결과 원문 18 PASS / 4 FAIL을 그대로 보존하고 대체 22/22 PASS. 빌드 exit 0, 기존 경고 6. 첫 구현 커밋의 부모와 기준선 일치 확인.
- 추가 증거: 주석 제외 SaveAnywhere 3행, 알림 금지 의존성 0건, 연결 diff +8/-0, AutoSaveNotice 2행/3회 출현. Check.md에 기록됨.
- 완료 기준 충족: 원문 계획은 보존했으며 재현 가능한 검증 경로와 실제 결과가 커밋됐다. 이 해결은 검증 명세에만 해당하며 Unity Play 모드 통과를 의미하지 않는다.
